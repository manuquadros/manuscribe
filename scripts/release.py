"""Cut the next manuscribe release: changelog section, commit, annotated tag, push.

git-cliff picks the version and renders `CHANGELOG.md`; this sequences
the git side around it. The tag is the version — `pdm-backend`'s scm
version source reads it at build time and `pyproject.toml` carries no
number — so creating the tag is what makes a release.
...
INTERNAL_RE = re.compile(r"^(build|chore|test|docs|ci)(\([^)]*\))?:")
...
def _nothing_to_release(tag: str | None, new: str, version: str | None) -> str | None:
    """A message if there is nothing to cut, else `None`."""
    if new == tag:
        return f"release: nothing to release — no commits since {tag}"
    if version is None and _only_internal(tag):
        return (
            f"release: nothing to release — every commit since {tag} is"
            " build/chore/test/docs/ci. Force one with --version vX.Y.Z"
        )
    return None


def _preflight(version: str | None) -> tuple[str | None, str] | None:
    """The current tag and the version to cut, or `None` if there is
    nothing to release (a message has already been printed)."""
    ...
    if (message := _nothing_to_release(tag, new, version)) is not None:
        print(message)
        return None
...
def release(*, dry_run: bool, push: bool, version: str | None) -> int:
    if (preflight := _preflight(version)) is None:
        return 1
    tag, new = preflight
    ...
    try:
        with _message_file(f"manuscribe {new.lstrip('v')}\n\n{section}\n") as msg:
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="release", description="Cut the next manuscribe release."
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
