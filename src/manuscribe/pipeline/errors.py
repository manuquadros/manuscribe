...
(:mod:`~manuscribe.pipeline.model`, :mod:`~manuscribe.pipeline.render`) and the
...
class ManuscribeError(Exception):
    """Base class for every error manuscribe raises at its public boundary."""


class OcrUnavailableError(ManuscribeError):
...
class PdfInputError(ManuscribeError):
